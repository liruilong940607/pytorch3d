...
template GLOBAL void calc_gradients_sampling_nerf<ISONDEVICE>(
    ...
    float const* const RESTRICT bg_col, /** Background color. */
...
template GLOBAL void calc_gradients_nerf<ISONDEVICE>(
    const CamInfo cam, /** Camera in world coordinates. */
    float const* const RESTRICT grad_im, /** The gradient image. */
    const float
        gamma, /** The transparency parameter used in the forward pass. */
